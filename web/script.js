...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const animationColorPicker = document.getElementById("animation-color-picker")
  ...
  async function removeIP(deviceIP, listItem) {
    ...
    eel.remove_device_from_dict(deviceIP)((response) => {
      if (response.success) {
        ...
        showNotification(`Failed to remove ${deviceIP}: ${response.message}`, "error")
      }
    })
  }
  ...
  function addDeviceToList(deviceIP) {
    ...
    removeBtn.addEventListener("click", () => {
      ...
      showConfirmationDialog(`Are you sure you want to remove ${deviceIP}?`, () => {
  ...
  function calculateCellSize(container, columns, rows) {
    ...
    const cellWidthByColumns = Math.floor((containerWidth - (columns - 1) * gapSpace) / columns)
    const cellHeightByRows = Math.floor((containerHeight - (rows - 1) * gapSpace) / rows)

    // Use the smaller dimension to ensure square cells that fit in the container
    // Ensure minimum size of 10px
    return Math.max(10, Math.floor(Math.min(cellWidthByColumns, cellHeightByRows)))
  }
  ...
  function handleGridCellClick() {
    ...
    if (gridData[index].assigned) {
      showConfirmationDialog(`Do you want to remove ${gridData[index].deviceIP} from grid?`, () => {
  ...
  function paintCell(index) {
    ...
    if (gridData[index].assigned) {
      eel.update_device_color(index, gridData[index].color, gridData[index].deviceIP)
    }
  }
  ...
  function paintAnimationCell(index) {
    ...
    const cell = document.querySelector(`.frames-grid .pixel-cell[data-index="${index}"]`)
    if (cell) {
  ...
  function updateFrameDisplay() {
    if (currentFrameDisplay) {
      currentFrameDisplay.textContent = `Frame: ${currentFrameIndex + 1} / ${frames.length}`
    }
  }
  ...
  function startAnimation() {
    ...
    const speedMultiplier = Number.parseFloat(document.querySelector(".speed-btn.active").dataset.speed) || 1.0
    ...
    return new Promise((resolve) => {
      eel.add_device_to_dict(ip)((response) => {
        if (response.success) {
          ...
          // Only show notification if not silent
          if (!silent) {
            showNotification(`Failed to add ${ip}: ${response.message}`, "error")
          }
          resolve(false)
        }
      })
  ...
  async function addDevices(options = { random: false }) {
    if (options.random) {
      ...
      if (!amount || isNaN(amount) || amount <= 0) {
        showNotification("Please enter a valid number of IPs to generate", "error")
        return
      }
      ...
      showNotification(`Added ${successCount} of ${amount} random IPs`, "success")
    } else {
  ...
  function deleteCurrentFrame() {
    ...
    showConfirmationDialog("Are you sure you want to delete this frame?", () => {
